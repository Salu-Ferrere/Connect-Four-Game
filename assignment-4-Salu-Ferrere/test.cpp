...
// test for connect 5 of diagonal, horizontal and vertical
TestResult test_SuperGameMoveExample2() {
	    Grid* grid = new Grid(6, 7);
	    SuperGame game;
	    game.setGrid(grid);
	    Player p1("Nick");
	    ...
	    // play winning move
	    ASSERT(game.playNextTurn(0));//2
	    ASSERT(game.playNextTurn(3));//1
	    ASSERT(game.playNextTurn(6)); //2
	    ASSERT(game.playNextTurn(2));//1
	    ASSERT(game.playNextTurn(4));//2
	    ASSERT(game.playNextTurn(2));//1
	    ASSERT(game.playNextTurn(1)); // p2
...
//test opposite diagonal 5
TestResult test_SuperGameMoveExample3() {
	    Grid* grid = new Grid(6, 7);
	    SuperGame game;
	    game.setGrid(grid);
	    Player p1("Nick");
	    ...
	    // | |x|o|o|x|o| |
	    // |x|o|o|x|x|o| |
	    ASSERT(game.playNextTurn(0)); // p1 x
	    ASSERT(game.playNextTurn(1)); // p2 o
	    ASSERT(game.playNextTurn(1)); // p1 x
	    ASSERT(game.playNextTurn(2)); // p2 o
	    ASSERT(game.playNextTurn(3)); // p1 x
	    ASSERT(game.playNextTurn(2)); // p2 o
	    ASSERT(game.playNextTurn(2)); // p1 x
	    ASSERT(game.playNextTurn(3)); // p2 o
	    ASSERT(game.playNextTurn(4)); // p1 x
	    ASSERT(game.playNextTurn(3)); // p2 o
	    ASSERT(game.playNextTurn(4)); // p1 x
	    ASSERT(game.playNextTurn(4)); // p2 o
	    ASSERT(game.playNextTurn(4)); // p1 x
	    ASSERT(game.playNextTurn(5)); // p2 o
	    ASSERT(game.playNextTurn(4)); // p1 x
	    ASSERT(game.playNextTurn(5)); // p2 o
	    std::string gridStateInitial = "       "
	                                   "    1  "
...
vector<TestResult (*)()> generateTests() {
    ...
    tests.push_back(&test_SuperGameMoveExample2);
    tests.push_back(&test_SuperGameMoveExample3);
#endif /*ENABLE_T4_TESTS*/
